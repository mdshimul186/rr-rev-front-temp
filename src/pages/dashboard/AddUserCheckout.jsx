...
import { addUserCheckout } from "../../services/subscription";
...
/**
 * AddUser Checkout component handles the checkout process for adding additional users.
 * It collects payment information and billing details, and processes the payment through Stripe.
 *
 * @returns {JSX.Element} The rendered checkout form for adding users.
 */
export const AddUserCheckout = () => {
  const { collapse, t } = useOutletContext();
  const navigate = useNavigate();
  const location = useLocation();
  const addUserPrice = location.state.price;
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        const updatedCardInfo = {
          ...
          amount: addUserPrice,
          additional_employee: number,
        };
        const response = await addUserCheckout(updatedCardInfo);
          ...
          <div className="dashboard-section ">
            ...
            <div className="billing-detail mt-2">
              <div>
                ...
                <span className="ms-auto large-price">${addUserPrice}</span>
              </div>
            </div>
          </div>
          <div className="dashboard-section">
            ...
            <div className="business-btns billing-btns gap-3">
              ...
              >
                {stripeIsLoading ? pending_loader : `${Pay} ${addUserPrice}`}
                {/* {Pay} ${addUserPrice} */}
              </button>
              <button
                type="button"
